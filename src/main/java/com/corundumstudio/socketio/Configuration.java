...
public class Configuration {
  ...
  private HttpRequestDecoderConfiguration httpRequestDecoderConfiguration =
      ...
      try {
        ...
        try {
          Class<?> jjs =
              getClass()
                  .getClassLoader()
                  .loadClass("com.corundumstudio.socketio.protocol.JacksonJsonSupport");
  ...
  /**
   * Allow to service custom requests differs from socket.io protocol. In this case it's necessary
   * to add own handler which handle them to avoid hang connections. Default is {@code false}
   *
   ...
   *
   * <p>With defined package prefix socket.io client just need to define '@class: 'SomeType'' in
   * json object instead of '@class: 'com.full.package.name.SomeType''
   *
  ...
  /**
   * Buffer allocation method used during packet encoding. Default is {@code true}
   *
   * @param preferDirectBuffer {@code true} if a direct buffer should be tried to be used as target
   *     for the encoded messages. If {@code false} is used it will allocate a heap buffer, which is
  ...
  /**
   * Data store - used to store session data and implements distributed pubsub. Default is {@code
   * MemoryStoreFactory}
   *
  ...
  /**
   * Authorization listener invoked on every handshake. Accepts or denies a client by {@code
   * AuthorizationListener.getAuthorizationResult} method. <b>Accepts</b> all clients by default.
   *
  ...
  /**
   * Set <b>Access-Control-Allow-Origin</b> header value for http each response. Default is <code>
   * null</code> If value is <code>null</code> then request <b>ORIGIN</b> header value used.
   *
   * @param origin - origin
   */
  ...
  /**
   * Timeout between channel opening and first data transfer Helps to avoid 'silent channel' attack
   * and prevents 'Too many open files' problem in this case
   *
  ...
  /**
   * Activate http protocol compression. Uses {@code gzip} or {@code deflate} encoding choice
   * depends on the {@code "Accept-Encoding"} header value.
   *
   * <p>Default is <code>true</code>
   *
  ...
  /**
   * Activate websocket protocol compression. Uses {@code permessage-deflate} encoding only.
   *
   * <p>Default is <code>true</code>
   *
  ...
  /**
   * Enable/disable client authentication. Has no effect unless a trust store has been provided.
   *
  ...
  public void setHttpRequestDecoderConfiguration(
