...
public class AuthorizeHandler extends ChannelInboundHandlerAdapter implements Disconnectable {
  ...
  private boolean authorize(
    ...
    HandshakeData data =
        new HandshakeData(
            ...
            req.uri(),
            origin != null && !"null".equalsIgnoreCase(origin));

    boolean result = false;
    Map<String, Object> storeParams = Collections.emptyMap();
    try {
      AuthorizationResult authResult =
          configuration.getAuthorizationListener().getAuthorizationResult(data);
    ...
    if (configuration.getTransports().contains(Transport.WEBSOCKET)
        && !(EngineIOVersion.V4.equals(client.getEngineIOVersion())
            && Transport.WEBSOCKET.equals(client.getCurrentTransport()))) {
      transports = new String[] {"websocket"};
    }

    AuthPacket authPacket =
        new AuthPacket(
            sessionId, transports, configuration.getPingInterval(), configuration.getPingTimeout());
    ...
    log.debug(
        "Handshake authorized for sessionId: {}, query params: {} headers: {}",
        sessionId,
        params,
        headers);
    return true;
  ...
  public void connect(ClientHead client) {
    ...
    if (!client.getNamespaces().contains(ns)) {
      ...
      if (!EngineIOVersion.V4.equals(client.getEngineIOVersion())) client.send(packet);
