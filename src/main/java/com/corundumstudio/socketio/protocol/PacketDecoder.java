...
public class PacketDecoder {
  ...
  public ByteBuf preprocessJson(Integer jsonIndex, ByteBuf content) throws IOException {
    String packet =
        URLDecoder.decode(content.toString(CharsetUtil.UTF_8), CharsetUtil.UTF_8.name());

    if (jsonIndex != null) {
      /**
       * double escaping is required for escaped new lines because unescaping of new lines can be
       * done safely on server-side (c) socket.io.js
       *
  ...
  private Packet addAttachment(ClientHead head, ByteBuf frame, Packet binaryPacket)
      throws IOException {
    ...
    if (binaryPacket.isAttachmentsLoaded()) {
      ...
      for (int i = 0; i < binaryPacket.getAttachments().size(); i++) {
        ...
        if (pos == -1) {
          scanValue =
              Unpooled.copiedBuffer("{\"num\":" + i + ",\"_placeholder\":true}", CharsetUtil.UTF_8);
    ...
    if (packet.getType() == PacketType.MESSAGE) {
      if (packet.getSubType() == PacketType.CONNECT
        ...
        if (packet.getSubType() == PacketType.CONNECT && frame.readableBytes() > 0) {
          final Object authArgs =
              jsonSupport.readValue(packet.getNsp(), new ByteBufInputStream(frame), Map.class);
  ...
  private String readNamespace(ByteBuf frame, final boolean defaultToAll) {
    ...
    /**
     * namespace post request with url queryString, like /message (v1) /message?a=1, (v2) /message,
     * (v3,v4)
     */
