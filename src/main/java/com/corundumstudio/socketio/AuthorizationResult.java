...
public class AuthorizationResult {
  ...
  public AuthorizationResult(boolean isAuthorized, Map<String, Object> storeParams) {
    this.isAuthorized = isAuthorized;
    this.storeParams =
        isAuthorized && storeParams != null
            ? Collections.unmodifiableMap(storeParams)
  ...
  /**
   * @return key-value pairs (unmodifiable) that will be added to {@link SocketIOClient } store. If
   *     a client is not authorized, storeParams will always be ignored (empty map)
   */
