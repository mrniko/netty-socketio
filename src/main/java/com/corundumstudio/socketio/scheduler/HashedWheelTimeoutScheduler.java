 ...
 */
/**
 * Modified version of HashedWheelScheduler specially for timeouts handling. Difference: - handling
 * old timeout with same key after adding new one fixes multithreaded problem that appears in highly
 * concurrent non-atomic sequence cancel() -> schedule()
 *
 * <p>(c) Alim Akbashev, 2015-02-11
 */
...
public class HashedWheelTimeoutScheduler implements CancelableScheduler {
  ...
  private final ConcurrentMap<SchedulerKey, Timeout> scheduledFutures =
  ...
  public void scheduleCallback(
      final SchedulerKey key, final Runnable runnable, long delay, TimeUnit unit) {
    Timeout timeout =
        executorService.newTimeout(
            new TimerTask() {
              @Override
