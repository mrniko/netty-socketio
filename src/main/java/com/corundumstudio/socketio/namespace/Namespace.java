...
public class Namespace implements SocketIONamespace {
  ...
  private final Queue<EventInterceptor> eventInterceptors =
      new ConcurrentLinkedQueue<EventInterceptor>();
  ...
  private final ConcurrentMap<String, Set<UUID>> roomClients =
      PlatformDependent.newConcurrentHashMap();
  private final ConcurrentMap<UUID, Set<String>> clientRooms =
      PlatformDependent.newConcurrentHashMap();
  ...
  public void onDisconnect(SocketIOClient client) {
    ...
    storeFactory
        .pubSubStore()
        .publish(
            PubSubType.BULK_LEAVE,
            new BulkJoinLeaveMessage(client.getSessionId(), roomsToLeave, getName()));
  ...
  public void onConnect(SocketIOClient client) {
    ...
    storeFactory
        .pubSubStore()
        .publish(
            PubSubType.JOIN, new JoinLeaveMessage(client.getSessionId(), getName(), getName()));
  ...
  public BroadcastOperations getBroadcastOperations() {
    return new SingleRoomBroadcastOperations(
  ...
  public BroadcastOperations getRoomOperations(String... rooms) {
    ...
    for (String room : rooms) {
      list.add(
          new SingleRoomBroadcastOperations(getName(), room, getRoomClients(room), storeFactory));
    }
  ...
  public void joinRoom(String room, UUID sessionId) {
    ...
    storeFactory
        .pubSubStore()
        .publish(PubSubType.JOIN, new JoinLeaveMessage(sessionId, room, getName()));
  }
  ...
  public void joinRooms(Set<String> rooms, final UUID sessionId) {
    ...
    storeFactory
        .pubSubStore()
        .publish(PubSubType.BULK_JOIN, new BulkJoinLeaveMessage(sessionId, rooms, getName()));
  }
  ...
  public void leaveRoom(String room, UUID sessionId) {
    ...
    storeFactory
        .pubSubStore()
        .publish(PubSubType.LEAVE, new JoinLeaveMessage(sessionId, room, getName()));
  }
  ...
  public void leaveRooms(Set<String> rooms, final UUID sessionId) {
    ...
    storeFactory
        .pubSubStore()
        .publish(PubSubType.BULK_LEAVE, new BulkJoinLeaveMessage(sessionId, rooms, getName()));
  }
