...
public class SocketIOChannelInitializer extends ChannelInitializer<Channel>
  ...
  public void start(Configuration configuration, NamespacesHub namespacesHub) {
    ...
    webSocketTransport =
        new WebSocketTransport(isSsl, authorizeHandler, configuration, scheduler, clientsBox);

    PacketListener packetListener =
        new PacketListener(ackManager, namespacesHub, xhrPollingTransport, scheduler);

    packetHandler =
        new InPacketHandler(
            packetListener, decoder, namespacesHub, configuration.getExceptionListener());
  ...
  protected void addSocketioHandlers(ChannelPipeline pipeline) {
    pipeline.addLast(
        HTTP_REQUEST_DECODER, new HttpRequestDecoder(configuration.getHttpDecoderConfig()));
    pipeline.addLast(
        HTTP_AGGREGATOR,
        new HttpObjectAggregator(configuration.getMaxHttpContentLength()) {
          @Override
  ...
  private SSLContext createSSLContext(Configuration configuration) throws Exception {
    ...
    KeyManagerFactory kmf =
        KeyManagerFactory.getInstance(configuration.getKeyManagerFactoryAlgorithm());
