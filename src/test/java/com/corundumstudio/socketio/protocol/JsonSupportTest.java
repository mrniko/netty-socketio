...
public class JsonSupportTest extends BaseProtocolTest {
  ...
  @Mock private AckCallback<String> ackCallback;
  ...
  public void testReadAckArgs() throws IOException {
    // Setup
    ByteBufInputStream inputStream =
        new ByteBufInputStream(Unpooled.wrappedBuffer("test".getBytes()));
  ...
  public void testAddEventMappingWithMultipleClasses() {
    ...
    doNothing()
        .when(jsonSupport)
        .addEventMapping(namespaceName, eventName, eventClass1, eventClass2);
  ...
  public void testGetArrays() {
    // Setup
    List<byte[]> expectedArrays = Arrays.asList("array1".getBytes(), "array2".getBytes());
